...
-----------------------------------------------------------------------------
{-# LANGUAGE CPP #-}
module Haddock.Backends.Xml (
  ppXml, copyXmlBits,
  ppXmlIndex, ppXmlContents,
) where
...
ppXml  dflags doctitle maybe_package ifaces odir prologue
  ...
  when (isNothing maybe_contents_url) $
    ppXmlContents dflags odir doctitle maybe_package
  ...
  when (isNothing maybe_index_url) $
    ppXmlIndex odir doctitle maybe_package
...
copyXmlBits :: FilePath -> FilePath -> IO ()
copyXmlBits odir libdir = do
  let
...
headXml :: String -> Maybe String -> Html
...
contentsButton maybe_contents_url
  ...
  where url = fromMaybe contentsXmlFile maybe_contents_url


indexButton :: Maybe String -> Maybe Html
indexButton maybe_index_url
  = Just (anchor ! [href url] << "Index")
  where url = fromMaybe indexXmlFile maybe_index_url


bodyXml :: String -> Maybe Interface
    -> SourceURLs -> WikiURLs
    -> Maybe String -> Maybe String
    -> Html -> Html
bodyXml doctitle iface
           maybe_source_url maybe_wiki_url
...
ppXmlContents
   ...
   -> IO ()
ppXmlContents dflags odir doctitle _maybe_package
  maybe_index_url
  ...
  let tree = mkModuleTree dflags showPkgs
      ...
      html =
        headXml doctitle Nothing +++
        bodyXml doctitle Nothing
          maybe_source_url maybe_wiki_url
  ...
  writeFile (joinPath [odir, contentsXmlFile]) (renderToString debug html)

  -- XXX: think of a better place for this?
  ppXmlContentsFrame odir doctitle ifaces debug
...
flatModuleTree ifaces =
  ...
  where
    ...
    ppModule' txt mdl =
      anchor ! [href (moduleXmlFile mdl), target mainFrameName]
        << toHtml txt


ppXmlContentsFrame :: FilePath -> String
  -> [InstalledInterface] -> Bool -> IO ()
ppXmlContentsFrame odir doctitle ifaces debug = do
  ...
  createDirectoryIfMissing True odir
  writeFile (joinPath [odir, frameIndexXmlFile]) (renderToString debug html)
...
ppXmlIndex odir doctitle _maybe_package 
  ...
  when split_indices $ do
    ...
    writeFile (joinPath [odir, subIndexXmlFile merged_name]) (renderToString debug mergedhtml)

  writeFile (joinPath [odir, indexXmlFile]) (renderToString debug html)

  where
    indexPage showLetters ch items =
      headXml (doctitle ++ " (" ++ indexName ch ++ ")") Nothing +++
      bodyXml doctitle Nothing
        maybe_source_url maybe_wiki_url
    ...
    indexInitialLetterLinks =
      divAlphabet <<
         unordList (map (\str -> anchor ! [href (subIndexXmlFile str)] << str) $
                        [ [c] | c <- initialChars
    ...
    do_sub_index this_ix c
      = unless (null index_part) $
          writeFile (joinPath [odir, subIndexXmlFile [c]]) (renderToString debug html)
      where
...
ppXmlModule odir doctitle 
  ...
  let
      ...
      html =
        headXml mdl_str (Just $ "mini_" ++ moduleXmlFile mdl) +++
        bodyXml doctitle (Just iface)
          maybe_source_url maybe_wiki_url
          maybe_contents_url maybe_index_url << [
